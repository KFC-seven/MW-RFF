...
def generate_tx_datasets(compact_dataset, tx_list, rx_list, capture_date_list, max_sig=None, equalized=1):
    """
    为每个发射机生成一个单独的数据集。
    
    参数：
    - compact_dataset: 原始数据集字典
    - tx_list: 发射机列表
    - rx_list: 接收机列表
    - capture_date_list: 采集日期列表
    - max_sig: 最大信号数量（可选）
    - equalized: 均衡标志（0 或 1）
    
    返回：
    - tx_datasets: 一个字典，键是发射机编号，值是该发射机对应的数据集
    """
    tx_datasets = {}  # 用于存储每个发射机的数据集
    
    # 遍历所有发射机
    for tx in tx_list:
        # 为每个发射机创建一个空字典
        dataset = {}
        dataset['node_list'] = [tx]  # 当前发射机的节点列表
        
        # 创建一个与接收机数量相同的空数据列表
        dataset['data'] = [() for _ in range(len(rx_list))]
        ...
        for capture_date in capture_date_list:
            # 遍历所有接收机
            for rx_train in rx_list:
                # 获取发射机、接收机、日期和均衡标志的索引
                tx_i = compact_dataset['tx_list'].index(tx)
                rx_i = compact_dataset['rx_list'].index(rx_train)
                date_i = compact_dataset['capture_date_list'].index(capture_date)
                eq_i = compact_dataset['equalized_list'].index(equalized)
                
                # 提取相应的数据，并将其添加到 dataset['data'] 中
                dataset['data'][rx_i] += (compact_dataset['data'][tx_i][rx_i][date_i][eq_i][slc],)
        
        # 将每个接收机的数据合并为一个 NumPy 数组
        for rx_idx in range(len(rx_list)):
            if len(dataset['data'][rx_idx]) > 0:
                dataset['data'][rx_idx] = np.concatenate(dataset['data'][rx_idx])
            else:
                dataset['data'][rx_idx] = np.zeros((0, 256, 2))  # 如果没有数据，填充空数组
        
        # 将当前发射机的数据集存储到 tx_datasets 字典中
        tx_datasets[tx] = dataset
